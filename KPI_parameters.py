        ...
        def sendkpiEmail(taskdetails):
            ...
            try:
                ...
                payload = json.dumps({
                    ...
                    'to': ['sairam.g@exactspace.co','ashlin.f@exactspace.co','shashank.r@exactspace.co'],
                    'html': msg_body,
                    'bcc': [],
...
for hour in [7,15,17,23]:
    scheduler.add_job(run_shiftwise, trigger='cron', hour=hour, minute=53, second=0)
for hour in [0,8,16,17]:
    scheduler.add_job(close_task, trigger='cron', hour=hour, minute=55, second=0)
